...
impl ImageExporter {
    pub fn export(html: &str, output: &std::path::Path) -> Result<(), MarkdownError> {
        ...
        let img_data = tab.capture_screenshot(format, None, None, true)
    ...
    fn set_viewport(tab: &headless_chrome::Tab, height: u32) -> Result<(), MarkdownError> {
        ...
        tab.call_method(Emulation::SetDeviceMetricsOverride {
            ...
            dont_set_visible_size: None, screen_orientation: None, viewport: None, display_feature: None, device_posture: None,
        }).map_err(|e| MarkdownError::ExportFailed(e.to_string()))?;
        Ok(())
    }
    ...
    fn determine_format(output: &std::path::Path) -> headless_chrome::protocol::cdp::Page::CaptureScreenshotFormatOption {
        use headless_chrome::protocol::cdp::Page::CaptureScreenshotFormatOption;
        let ext = output.extension().and_then(|s| s.to_str()).map(|s| s.to_lowercase()).unwrap_or_default();
        if ext == "jpg" || ext == "jpeg" { CaptureScreenshotFormatOption::Jpeg } else { CaptureScreenshotFormatOption::Png }
    }
