    ...
    if !text.is_empty() {
        ...
        let copy_button = ui
            .put(
                ...
                egui::Button::image(if copied_icon {
                    egui::Image::new("bytes://icon/success.svg").tint(ui.visuals().text_color())
                } else {
                    egui::Image::new("bytes://icon/copy.svg").tint(ui.visuals().text_color())
                })
                .small()
                .frame(false)
