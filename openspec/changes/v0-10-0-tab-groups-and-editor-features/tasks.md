...
- [ ] 2.5 設定画面に「前回のタブを復元する」ON/OFFトグルを追加
...
- [ ] 3.1 ピン留め中のタブの閉じるボタン（×）を非表示にする
- [ ] 3.2 ピン留め中でもホバー時にタイトルをツールチップで表示する
- [ ] 3.3 ショートカットキーでのタブ閉じ操作がピン留めタブに無効になるよう実装
- [ ] 3.4 コンテキストメニューの「ピン留め解除」実装を確認・修正（解除後に×ボタン再表示）
