...
builder_rs_path = "crates/katana-platform/src/theme/builder.rs"
palettes_rs_path = "crates/katana-platform/src/theme/palettes.rs"

with open(builder_rs_path, "r") as f:
    builder_content = f.read()

# We extract all consts at the top.
# The consts are from line 5 to 52. 
palettes_content = """//! Shared color palette and alpha constants for theme builders.
...
pub(crate) const DEFAULT_SUCCESS_DARK: Rgb = Rgb { r: 153, g: 204, b: 153 };
pub(crate) const DEFAULT_SUCCESS_LIGHT: Rgb = Rgb { r: 60, g: 130, b: 60 };
pub(crate) const DEFAULT_WARNING_DARK: Rgb = Rgb { r: 255, g: 204, b: 102 };
pub(crate) const DEFAULT_WARNING_LIGHT: Rgb = Rgb { r: 160, g: 110, b: 20 };
pub(crate) const DEFAULT_ERROR_DARK: Rgb = Rgb { r: 242, g: 119, b: 122 };
pub(crate) const DEFAULT_ERROR_LIGHT: Rgb = Rgb { r: 180, g: 40, b: 40 };
"""

# Now write builder_content replacing the old consts with use palettes::*
lines = builder_content.splitlines()

# remove lines 5 through 52
new_lines = []
...
new_builder_content = "\\n".join(new_lines)
new_builder_content = new_builder_content.replace(
    "use crate::theme::types::{",
    "use crate::theme::palettes::*;\\nuse crate::theme::types::{"
)

with open(palettes_rs_path, "w") as f:
    f.write(palettes_content)

with open(builder_rs_path, "w") as f:
