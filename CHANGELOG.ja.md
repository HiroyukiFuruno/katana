...
- **OpenSpec ワークフロー連携**: `openspec instructions` CLIに基づく標準化された提案、設計、タスクのアーティファクト仕様駆動開発フローを導入。

### 🐛 バグ修正

- **高負荷とフリーズ**: アイドル時のCPU 100%過負荷およびUIのスピナー無限ループ問題（描画ハッシュ、SVGロード起因）を修正 (P0)。
- **結合テストの安定化**: テスト実行環境に限りスプラッシュ画面をスキップさせ、意図しないテスト失敗（偽陽性）を防止 (P1)。
- **プレビュー用欠落アイコン**: SVG ローダーに `bytes://` および `data:` URI (Base64/パーセントエンコーディング) 解析機能を追加し、アイコンがロードできないエラーを解消 (P4)。
